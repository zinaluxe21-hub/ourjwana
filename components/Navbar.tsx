...
const Navbar: React.FC = () => {
  ...
  // Sur l'accueil : dépend du scroll. Sur les autres pages : TOUJOURS solide.
    ...
    <>
      <nav 
        className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 px-6 md:px-12 ${
          isSolid ? 'bg-white shadow-md py-3' : 'bg-transparent py-6'
        }`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className={`p-2 transition-colors ${isSolid ? 'text-[#C15B36]' : 'text-white'} hover:text-[#C15B36]`}
            >
              <Menu size={28} />
            </button>
          </div>
        ...
        {isMenuOpen && (
          ...
          >
            <button 
              ...
              className="absolute top-8 right-8 text-[#2C3E50] p-2 hover:rotate-90 transition-transform duration-300"
            >
              <X size={40} />
            </button>
